...
class PincherController(Node):
    ...
    def __init__(self) -> None:
        ...
        # Publicar estados articulares iniciales para sincronizar RViz y robot_state_publisher inmediatamente
    ...
    def _connect_hardware(self) -> bool:
        try:
            ...
            if not self.port_handler.openPort():
                self.get_logger().error(f'❌ No se pudo abrir el puerto {self.port_name}. Verifica la conexión física del cable USB.')
                return False
            ...
            self.get_logger().error(f'❌ Error abriendo puerto serial {self.port_name}: {e}. Entrando en modo seguro.')
            return False
    ...
    def _command_pairs(self, msg: JointState) -> Iterable[Tuple[int, float]]:
        alias_map = {
            'waist': 0, 'arm_shoulder_pan': 0, 'arm_shoulder_pan_joint': 0,
            'shoulder': 1, 'arm_shoulder_lift': 1, 'arm_shoulder_lift_joint': 1,
            'elbow': 2, 'arm_elbow_flex': 2, 'arm_elbow_flex_joint': 2,
            'wrist': 3, 'arm_wrist_flex': 3, 'arm_wrist_flex_joint': 3,
            'gripper': 4, 'gripper_finger1': 4, 'gripper_finger1_joint': 4,
        }
        if msg.name:
            ...
            if not matched:
                for input_name, pos in name_to_position.items():
                    clean_name = input_name.replace('phantomx_pincher_', '')
                    if clean_name in alias_map:
                        yield alias_map[clean_name], float(pos)
                    elif input_name in alias_map:
                        yield alias_map[input_name], float(pos)
        else:
    ...
    def state_timer_callback(self) -> None:
        ...
        if len(positions) > 4:
            # Map gripper joint (index 4) to URDF prismatic meters:
            # 0° (0 rad) -> 0.0158m (Open)
            # 90° (1.57 rad) -> 0.001m (Closed)
            val = positions[4]
            deg = math.degrees(abs(val)) if abs(val) <= 2.0 * math.pi else abs(val)
            deg = max(0.0, min(90.0, deg))
            positions[4] = 0.0158 - (deg / 90.0) * (0.0158 - 0.001)
