...
# Limites conservadores para las primeras pruebas con el robot fisico. Estan
# tomados de las mediciones del equipo 3A sobre un PhantomX Pincher identico,
# reducidos un margen hacia adentro. Deben refinarse con nuestra propia medicion
# de topes (bringup/leer_posiciones.py) sobre nuestra unidad.
JOINT_LIMITS_DEG = {
    'waist': (-140.0, 140.0),
    'shoulder': (-70.0, 70.0),
    'elbow': (-115.0, 130.0),
...
class PincherGuiNode(Node):
    ...
    def __init__(self) -> None:
        ...
        # Nombres a publicar en /pincher/command. Por defecto se usan los mismos
        # de la GUI (waist, shoulder, ...); con este parametro se pueden remapear
        # a los del modelo en uso (p.ej. los del KIT) sin cambiar la interfaz.
        self.declare_parameter('joint_names', list(JOINT_LIMITS_DEG))
        self.published_names = [str(name)
    ...
    def publish_joint_command(self, names: List[str], degrees: List[float]) -> None:
        ...
        if len(self.published_names) == len(names):
