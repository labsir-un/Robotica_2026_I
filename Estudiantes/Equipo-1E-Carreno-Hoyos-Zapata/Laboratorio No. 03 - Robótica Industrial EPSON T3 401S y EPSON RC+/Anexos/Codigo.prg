...
Function IsReachable(name$ As String, idx As Integer) As Integer
    ...
    If name$ = "A" Then
        r1 = RowFromIndex(eggApos)
        C1 = ColFromIndex(eggApos)
    Else
        r1 = RowFromIndex(eggBpos)
        C1 = ColFromIndex(eggBpos)
    EndIf
    r2 = RowFromIndex(idx)
    C2 = ColFromIndex(idx)
    dr = Abs(r1 - r2)
...
Function MoveEgg(name$ As String, targetIdx As Short)
    If name$ = "A" Then
        Print "  >> Recogiendo huevo A en posicion:", eggApos
        Jump Pallet(1, eggApos)
        Wait 0.2
        Print "  >> Llevando huevo A a posicion:", targetIdx
        PickAndPlace("A", targetIdx)
        UpdateOcc("A", targetIdx)
        movimientos = movimientos + 1
        Print "  Movimientos realizados:", movimientos, " de 30"
    Else
        Print "  >> Recogiendo huevo B en posicion:", eggBpos
        Jump Pallet(1, eggBpos)
        Wait 0.2
        Print "  >> Llevando huevo B a posicion:", targetIdx
...
Function main
    ...
    Accel 50, 50
    Speed 30
    Print "Iniciando rutina - Patron caballo de ajedrez"
    Print "Huevo A posicion inicial: 1"
    Print "Huevo B posicion inicial: 30"
    Home
    Pallet 1, Origin, PuntoX, PuntoY, 6, 5
    Call InitTour
    Call InitOcc
    For sIndex = 1 To 30
        Print "--- Paso:", sIndex, " de 30 ---"
        If (sIndex Mod 2) = 1 Then
            Print "Turno: Huevo A -> posicion:", tour(sIndex)
            If eggApos <> tour(sIndex) Then
                Call MoveEgg("A", tour(sIndex))
            EndIf
        Else
            Print "Turno: Huevo B -> posicion:", tour(sIndex)
            If eggBpos <> tour(sIndex) Then
                Call MoveEgg("B", tour(sIndex))
            EndIf
        EndIf
    Next
    Print "Rutina completada!"
    Print "Total movimientos realizados:", movimientos, " de 30"
    Home
    Motor Off
Fend
