...
class PincherController(Node):
    ...
    def move_to_angles_degrees(self, angles_deg):
        ...
        if len(angles_deg) != 5:
            self.gest_logger().error(f'Se esperaban 5 ángulos, se recibieron {len(angles_deg)}')
            return False
    ...
    def rd_gripper(self, qg):

        th5=self.radians_to_dxl(qg)
        
        self.move_motor(5, th5)

        self.get_logger().info('El motor ha sido movido')

    def motors_forward(self, inp):

        th4=self.radians_to_dxl(inp)
        
        self.move_motor(2, th4)
