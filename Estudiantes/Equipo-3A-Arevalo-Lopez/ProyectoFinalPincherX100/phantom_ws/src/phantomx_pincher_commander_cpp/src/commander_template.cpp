                        ...
                        double roll, double pitch, double yaw, bool cartesian_path=false)
    {
        ...
        if (!cartesian_path) {
            ...
            if (arm_->setApproximateJointValueTarget(target_pose, "")) {
                ...
                // Always use elbow-up constraint with enough retries.
                // The constraint ensures the arm always moves "the right way".
                moveit_msgs::msg::Constraints constraints;
                moveit_msgs::msg::JointConstraint elbow_c;
                elbow_c.joint_name = "phantomx_pincher_arm_elbow_flex_joint";
                elbow_c.position = 1.0;
                elbow_c.tolerance_below = 1.5;   // allows [-0.5, 3.5] — very wide
                elbow_c.tolerance_above = 2.5;
                elbow_c.weight = 0.8;
                constraints.joint_constraints.push_back(elbow_c);
                arm_->setPathConstraints(constraints);

                planned = planAndExecuteWithRetries(arm_, 10);
                arm_->clearPathConstraints();

                if (!planned) {
                    RCLCPP_ERROR(node_->get_logger(),
                        "All planning attempts failed for pose (%.3f, %.3f, %.3f)", x, y, z);
                    if (is_home_pose && home_joint_fallback_) {
                        RCLCPP_INFO(node_->get_logger(), "HOME fallback: moving joints to zeros.");
                        std::vector<double> home_joints = {0.01745, 0.01745, 0.01745, 0.01745};
...
private:
    ...
    bool planAndExecuteWithRetries(const std::shared_ptr<MoveGroupInterface> &interface, int max_attempts)
    {
        for (int i = 0; i < max_attempts; ++i) {
            ...
            if (success) {
                ...
                RCLCPP_WARN(node_->get_logger(), "Execution failed on attempt %d/%d", i + 1, max_attempts);
            } else {
                RCLCPP_WARN(node_->get_logger(), "Planning failed on attempt %d/%d", i + 1, max_attempts);
            }
        }
        return false;
    }
